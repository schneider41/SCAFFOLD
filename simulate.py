...
def simulate(num_of_users,dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L, num_glob_iters,
               local_epochs, optimizer, clients_per_round, rho, times, similarity):
    print("=" * 80)
    print("Summary of training process:")
    print(f"Algorithm: {algorithm}")
    print(f"Batch size              : {batch_size}")
    print(f"Learing rate            : {learning_rate}")
    print(f"Subset of users         : {clients_per_round}")
    ...
    for i in range(times):
        ...
        if (model == "cifar_cnn"):  # for Mnist and Femnist datasets
            model = CifarNet(), model

        # select algorithm
        if (algorithm == "FedAvg"):
            server = FedAvg(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L,
                            num_glob_iters, local_epochs, optimizer, clients_per_round, rho, similarity, i)

        if (algorithm == "FEDL"):
            server = FEDL(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L, num_glob_iters,
                          local_epochs, optimizer, clients_per_round, rho, i)

        if (algorithm == "SCAFFOLD"):
            server = SCAFFOLD(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L,
                              num_glob_iters, local_epochs, optimizer, clients_per_round, similarity, i)
        server.train()
        server.test()

    # Average data
    average_data(clients_per_round=clients_per_round, local_epochs=local_epochs, num_glob_iters=num_glob_iters,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="Femnist",
                        choices=["Mnist", "Femnist", "Linear_synthetic", "Logistic_synthetic"])
    ...
    parser.add_argument("--clients_per_round", type=int, default=20, help="Number of Users per round")
    parser.add_argument("--rho", type=float, default=0, help="Conditon Number")
    parser.add_argument("--times", type=int, default=1, help="running time")
    args = parser.parse_args()

    simulate(
        num_of_users=args.num_of_users,
